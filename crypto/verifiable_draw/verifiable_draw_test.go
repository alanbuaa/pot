...
func TestDrawProof_ToBytes_FromBytes(t *testing.T) {
	...
	fmt.Println("draw proof serialize and deserialize test")

	drawProofBytes, err := drawProof.ToBytes()
	if err != nil {
		fmt.Println("serialize draw proof error:", err)
		return
	}
	decodedDrawProof, err := new(DrawProof).FromBytes(drawProofBytes)
	if err != nil {
		fmt.Println("deserialize draw proof error:", err)
		return
	}
	fmt.Println("start verify draw proof")
	start = time.Now()

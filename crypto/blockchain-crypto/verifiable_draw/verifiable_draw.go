...
import (
	...
	"blockchain-crypto/proof/dleq/bls12381"
	"blockchain-crypto/proof/kzg_ped_linkability/kzg_vector"
...
func Draw(s *srs.SRS, candidatesNum uint32, candidatesPubKey []*PointG1, quota uint32, secretVector []uint32, prevRCommit *PointG1) (*DrawProof, error) {
	...
	caulkPlusProof, err := caulk_plus.CreateMultiProof(s, candidatesNum, vVector, quota, aVector)
	if err != nil {
		return nil, err
	}
...
func Verify(s *srs.SRS, candidatesNum uint32, candidatesPubKey []*PointG1, quota uint32, prevRCommit *PointG1, drawProof *DrawProof) error {
	...
	// verify caulk plus
	res, err := caulk_plus.VerifyMultiProof(s, drawProof.CaulkPlusProof)
	if !res {
		if err != nil {
...
func IsSelected(sk *Fr, rCommit *PointG1, selectedKeys []*PointG1) (bool, uint32, *PointG1) {
	group1 := NewG1()
	skInv := NewFr().Inverse(sk)
	keysLen := uint32(len(selectedKeys))
	for i := uint32(0); i < keysLen; i++ {
		if group1.Equal(rCommit, group1.MulScalar(group1.New(), selectedKeys[i], skInv)) {
			return true, i, selectedKeys[i]
		}
	}
	return false, 0, nil
}

...
// 	data, err := os.ReadFile("tests/g1_uncompressed_valid_test_vectors.dat")
// 	if err != nil {
// 		panic(err)
// 	}
// 	g := NewG1()
...
// 	data, err := os.ReadFile("tests/g1_compressed_valid_test_vectors.dat")
// 	if err != nil {
// 		panic(err)
// 	}
// 	g := NewG1()
...
func TestG1ToCompressedAndFromCompressed(t *testing.T) {
	g := NewG1()
	for i := 0; i < fuz; i++ {
		fr, _ := NewFr().Rand(rand.Reader)
		p0 := g.MulScalar(g.new(), g.One(), fr)
		p1, err := g.FromCompressed(g.ToCompressed(p0))
		if err != nil {
			t.Fatal("from compressed is failed err:", err)
		}
		if !g.Equal(p0, p1) {
			t.Fatal("to compressed and from compressed is failed err:", err)
		}
	}
}

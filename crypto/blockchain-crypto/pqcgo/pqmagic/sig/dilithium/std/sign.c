...
#else
  sm3_extended(seedbuf, 2*SEEDBYTES + CRHBYTES, seedbuf, SEEDBYTES);
#endif
  rho = seedbuf;
...
#else
  sm3_extended(tr, SEEDBYTES, pk, CRYPTO_PUBLICKEYBYTES);
#endif
  ...
  const uint8_t *sk)
{
  ...
  keccak_state state;
#else
  uint8_t *tr_msg = (uint8_t*)malloc(SEEDBYTES + mlen);
...
#else
  memcpy(tr_msg, tr, SEEDBYTES);
  memcpy(tr_msg + SEEDBYTES, m, mlen);
  sm3_extended(mu, CRHBYTES, tr_msg, SEEDBYTES + mlen);
#endif

  if(coins) {
    memcpy(rhoprime, coins, CRHBYTES);
  } else {
#ifdef USE_SHAKE
    shake256(rhoprime, CRHBYTES, key, SEEDBYTES + CRHBYTES);
#else
    sm3_extended(rhoprime, CRHBYTES, key, SEEDBYTES + CRHBYTES);
#endif
  }
...
rej:
  ...
  sm3_extended(sig, SEEDBYTES, mu_sig, CRHBYTES + K*POLYW1_PACKEDBYTES);
#endif
                          ...
                          const uint8_t *sk)
{
  ...
  uint8_t coins[CRHBYTES];
  randombytes(coins, CRHBYTES);
  return crypto_sign_signature_internal(sig, siglen, m, mlen, coins, sk);
#else
  return crypto_sign_signature_internal(sig, siglen, m, mlen, NULL, sk);
#endif
}
  ...
  const uint8_t *pk)
{
  ...
  shake256_squeeze(mu, CRHBYTES, &state);
#else
  sm3_extended(mu, SEEDBYTES, pk, CRYPTO_PUBLICKEYBYTES);
  memcpy(tr_msg, mu, SEEDBYTES);
  memcpy(tr_msg + SEEDBYTES, m, mlen);
  sm3_extended(mu, CRHBYTES, tr_msg, SEEDBYTES + mlen);
#endif
  ...
  sm3_extended(c2, SEEDBYTES, mu_buf, CRHBYTES + K*POLYW1_PACKEDBYTES);

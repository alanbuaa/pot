...
func Execute(challenge []byte, iterations int, ctrl *utils.Controller, cpuCounter *utils.CPUCounter) []byte {
	...
	// 执行命令
	command := utils.GetCurrentAbPathByCaller() + "/vdf-cli -tpietrzak " + hex.EncodeToString(challenge) + " " + strconv.Itoa(iterations)
...
func Verify(challenge []byte, iterations int, res []byte) bool {
	...
	// 执行命令
	command := utils.GetCurrentAbPathByCaller() + "/vdf-cli -tpietrzak " + hex.EncodeToString(challenge) + " " + strconv.Itoa(iterations) + " " + hexRes

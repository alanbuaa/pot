...
本文提出一种支持在线热升级的区块链共识协议切换机制(Switchable Consensus Upgrade Protocol, SCUP)。该协议允许区块链系统在运行时安全地从一种共识算法切换到另一种共识算法,无需停机或硬分叉。通过引入升级配置交易、多链预执行机制、治理委员会验证和共识描述语言(Consensus Description Language, CDL),本协议实现了共识算法的可插拔升级,同时保证系统的安全性、活性和一致性。多链机制允许在同一升级提案中同时执行多个候选共识,在确认阶段通过性能评估和治理投票选择最优的共识进行激活,从而实现更科学的升级决策。
...
1. **多链预执行机制**: 在保持主链稳定的同时,在多个独立分叉上并行预执行多个候选共识
2. **多候选共识并行**: 支持在单个升级提案中同时提议和评估多个候选共识算法
3. **性能驱动的共识选择**: 基于实际运行指标(吞吐量、延迟、错误率)选择最优共识
4. **多阶段升级流程**: 提案→预备→预执行→确认→切换的完整生命周期管理
5. **治理委员会验证**: 基于门限签名的去中心化治理机制  // 这里待定
6. **共识描述语言(CDL)**: 支持自定义共识算法的声明式描述框架
7. **形式化安全分析**: 提供完整的安全性证明和攻击模型分析
...
3. **预执行阶段分区** (最关键):
   - **问题 3.1**: 主链和候选链可能在不同分区产生冲突
   - **解决 3.1**: 候选链交易严格同步自主链,避免冲突
   
   - **问题 3.2**: 分区导致部分节点的候选链落后
   - **解决 3.2**: 分区恢复后,节点从连通节点同步候选链数据
   
   - **问题 3.3**: 分区期间主链和候选链分别在不同分区形成多数
...
class PartitionTolerantUpgrade:
    ...
    def handle_partition_during_preexec(self, network_state):
        ...
        # 规则 2: 候选链的交易完全来自主链
        # 即使候选链分区,分区恢复后可以根据主链重建
        def rebuild_candidate_chain(fork_point, current_height):
            """
            分区恢复后重建候选链
            """
            ...
            for h in range(fork_point, current_height + 1):
                ...
                # 使用主链交易在候选链上重新执行
                candidate_txs = main_block.body.transactions
                candidate_block = new_consensus.produce_block(candidate_txs)
                candidate_chain_new.append(candidate_block)
        ...
        if detect_partition_recovered():
            ...
            # 3.2 检查本地候选链是否与主链一致
            local_candidate_valid = validate_candidate_chain_against_main(
                local_candidate_chain, 
                canonical_main_chain
            )
            
            if not local_candidate_valid:
                # 3.3 丢弃本地错误的候选链,根据主链重建
                log.warn("候选链与主链不一致,重建中...")
                local_candidate_chain = rebuild_candidate_chain(
                    fork_point, 
...
def validate_candidate_chain_against_main(candidate_chain, main_chain):
    """
    验证候选链是否与主链一致
    """
    fork_point = candidate_chain.fork_point
    
    for h in range(fork_point, min(candidate_chain.height, main_chain.height) + 1):
        candidate_block = candidate_chain.get_block(h)
        main_block = main_chain.get_block(h)
...
层次 2: 候选链可恢复
- 候选链的输入(交易)完全来自主链
- 给定相同的主链状态,候选链是确定性可重建的
- 分区恢复后,节点可以根据权威主链重建候选链
- 因此,候选链在分区恢复后能达成一致 ✓
...
在多链预执行期间,系统同时运行旧共识和多个候选共识,因此必须满足:

```
Safety_upgrade = Safety_old ∪ Safety_candidate_1 ∪ Safety_candidate_2 ... ∪ Safety_candidate_N
```
...
- **超时参数**: 
  ...
  - T_preexec: 预执行阶段的最小区块数 (≥ 1000) // 大致要求候选链的运行时间
...
UpgradeConfigTx := {
    ...
    // 多候选共识支持
    candidateConsensuses: []CandidateConsensus {  // 候选共识列表 (1-N 个)
        {
            candidateID: Hash,            // 候选共识唯一标识
            consensusName: String,        // 共识名称 (内置 | 自定义)
            consensusDescriptor: CDL,     // 共识描述语言 (仅自定义时)
            descriptorHash: Hash,         // 描述符哈希 (仅自定义时)
            forkID: uint64,               // 分叉 ID，用于区分不同分叉
        }
    },
...
CandidateConsensus := {
    ...
    consensusDescriptor: CDL,
    descriptorHash: Hash,
    forkID: uint64,
    expectedPerformance: PerformanceMetrics,  // 期望性能指标
}
```
...
UpgradeConfirmTx := {
    ...
    selectedCandidateID: Hash,            // 选中的候选共识 ID
    candidateChainHeads: []ChainHead {    // 所有候选链的头部哈希
        {
            candidateID: Hash,
            chainHead: Hash,
            finalHeight: uint64,
            performanceScore: float64,     // 性能评分 (0-1)
        }
    },
...
为了确保候选链能够在升级确认后无缝成为主链,本协议采用**统一的区块格式**:
...
Block := {
    header: {
        ...
        // 升级相关字段 (可选字段，在主链区块中为空或标记为非候选链)
        upgradeMetadata: {
            isPreexec: bool,          // 是否为候选链区块
            proposalID: Hash,         // 关联的升级提案ID (候选时有效)
            candidateID: Hash,        // 候选共识 ID (预执行时有效)
            forkID: uint64,           // 分叉 ID，用于区分不同分叉 (预执行时有效)
...
1. **统一格式**: 主链和候选链使用完全相同的区块结构
2. **可选字段**: `upgradeMetadata` 在候选执行期间填充,在主链区块中为空或标记为非候选链
3. **共识中立**: `consensusProof` 字段可以容纳不同共识算法的证明(PoW nonce、BFT 签名、PoS 证明等)
4. **无缝合并**: 候选链直接成为主链区块,通过切换链的 head 指针实现
...
```python
def activate_candidate_chain_as_main(candidate_chain, fork_point, switch_height):
    """
    激活候选链为主链 (关键: 不修改任何区块内容)
    """
    # 1. 验证候选链的连续性和有效性
    for height in range(fork_point, switch_height + 1):
        ...
        assert validate_block(candidate_block, new_consensus) == true
        
        # 验证哈希链连续性
        if height > fork_point:
            prev_block = candidate_chain.get_block(height - 1)
            assert candidate_block.header.parentHash == hash(prev_block)
    ...
    atomic {
        ...
        # 2.2 将主链的 head 指针切换到候选链
        main_chain.head = candidate_chain.head
        main_chain.tip_hash = candidate_chain.tip_hash
        main_chain.consensus_id = candidate_chain.consensus_id
        
        # 2.3 更新区块索引
        #     将 [fork_point, switch_height] 高度映射到候选链区块
        for height in range(fork_point, switch_height + 1):
            candidate_block = candidate_chain.get_block(height)
            # 注意: candidate_block 保持原样,不做任何修改
            main_chain.height_to_block[height] = candidate_block
    ...
    assert main_chain.state_root == candidate_chain.state_root
...
def verify_fault_tolerance_compatibility(old_consensus, new_consensus, network_state):
    ...
    # 4. 验证预执行期间的安全性
    # 多链期间需要同时满足所有共识的约束
    multi_chain_safe = verify_multi_chain_safety(old_model, new_model, network_state)
    if not multi_chain_safe:
        return False, "多链预执行期间无法同时满足旧共识和候选共识的安全约束"
...
1. **多链并行运行**:
   ```python
   def run_multi_chain(prepare_height: int, candidates: []CandidateConsensus):
       main_chain = current_chain
       candidate_chains = {}
       
       # 为每个候选共识创建独立分叉
       for candidate in candidates:
           fork = fork_chain_at(prepare_height, candidate.forkID)
           candidate_chains[candidate.candidateID] = {
               'chain': fork,
               'consensus': load_consensus(candidate),
       ...
       while current_height < tx.preexecHeight:
           ...
           # 同步主链交易到所有候选链
           for candidate_id, chain_data in candidate_chains.items():
               # 同步交易
               candidate_txs = sync_transactions_from(main_block)
               
               # 使用候选共识生成区块
               candidate_block = chain_data['consensus'].produce_block(candidate_txs)
               candidate_block.header.upgradeMetadata = {
                   'isPreexec': True,
                   'proposalID': tx.proposalID,
                   'candidateID': candidate_id,
                   'forkID': candidates[candidate_id].forkID,
                   'forkPoint': prepare_height,
                   'mainChainRef': main_block.hash()
               }
               chain_data['chain'].append(candidate_block)
               
               # 收集性能指标
               metrics = collect_metrics(candidate_block)
               chain_data['metrics'].record(metrics)
   ```
...
2. **性能监控与评估**:
   ...
   ```python
   def evaluate_candidates(candidate_chains):
       scores = {}
       for candidate_id, chain_data in candidate_chains.items():
           metrics = chain_data['metrics'].summarize()
           
           # 综合评分公式 (可配置权重)
           score = (
               0.4 * metrics.throughput_score +      # 吞吐量权重
               0.3 * metrics.latency_score +         # 延迟权重
               0.2 * metrics.stability_score +       # 稳定性权重
               0.1 * metrics.resource_efficiency     # 资源效率权重
           )
           scores[candidate_id] = {
               'score': score,
               'metrics': metrics,
       ...
       for rank, (candidate_id, data) in enumerate(ranked, 1):
           scores[candidate_id]['rank'] = rank
...
1. **性能评估与排名**:
   ...
   ```python
   def submit_evaluation_to_committee(candidate_scores):
       evaluation = {
           'nodeID': self.node_id,
           'proposalID': tx.proposalID,
           'candidateScores': candidate_scores,
           'timestamp': now(),
           'signature': sign(candidate_scores)
       }
       broadcast_to_committee(evaluation)
   ```

2. **治理投票与选择**:
   - 委员会成员收集所有节点的评估结果
   - 基于预执行结果选择最优候选共识
   - 投票选项: APPROVE(批准,并指定 candidateID) | REJECT(拒绝所有) | ROLLBACK(回退)
   
   ```python
   def committee_vote_on_candidates(evaluations, candidate_chains):
       # 1. 聚合所有节点的评估
       aggregated_scores = aggregate_evaluations(evaluations)
       
       # 2. 计算每个候选的综合评分 (中位数或加权平均)
       final_scores = {}
       for candidate_id in candidate_chains.keys():
           scores = [e['candidateScores'][candidate_id]['score'] 
                    for e in evaluations]
           final_scores[candidate_id] = median(scores)
       
       # 3. 选择最高分的候选
       best_candidate = max(final_scores.items(), key=lambda x: x[1])
       selected_id, selected_score = best_candidate
       
       # 4. 验证选中的候选是否满足最低要求
       if selected_score < MIN_ACCEPTABLE_SCORE:
           return Vote(REJECT, reason="所有候选均未达最低标准")
       
       # 5. 检查安全性
       if not verify_safety(candidate_chains[selected_id]):
           return Vote(ROLLBACK, reason="安全性验证失败")
       
       # 6. 批准并指定选中的候选
       return Vote(APPROVE, selectedCandidateID=selected_id, score=selected_score)
   ```

3. **确认交易上链**:
   - 达到阈值后,生成 `UpgradeConfirmTx`
   - 必须包含选中的 `selectedCandidateID`
   - 必须在 `preexecHeight + T_confirm` 之前上链
   - 否则升级自动失败,回退到主链
...
1. **切换准备**:
   ```python
   def prepare_switch(confirm_tx: UpgradeConfirmTx, candidate_chains):
       # 确定选中的候选共识
       selected_id = confirm_tx.selectedCandidateID
       selected_chain = candidate_chains[selected_id]
       ...
       atomic {
           ...
           # 2. 合并选中的候选链
           merge_candidate_chain(selected_chain, switch_height)
           
           # 3. 清理其他未选中的候选链
           for candidate_id, chain in candidate_chains.items():
               if candidate_id != selected_id:
                   cleanup_candidate_chain(candidate_id, chain)
           
           # 4. 激活选中的新共识
           selected_consensus = selected_chain['consensus']
           selected_consensus.start()
           
           # 5. 更新全局状态
           current_consensus = selected_consensus
           current_consensus_id = selected_id
       }
   ```
...
2. **状态迁移**:
   ...
   ```python
   def activate_upgrade(selected_chain, other_chains, fork_point, switch_height):
       """
       激活升级,将选中的候选链设为主链 (不修改任何区块内容)
       """
       selected_id = selected_chain['candidateID']
       selected_fork = selected_chain['chain']
       
       # 1. 验证选中的候选链完整性
       for h in range(fork_point, switch_height + 1):
           block = selected_fork.get_block(h)
           
           # 验证是候选链区块
           assert block.header.upgradeMetadata.isPreexec == true
           assert block.header.upgradeMetadata.candidateID == selected_id
           assert block.header.upgradeMetadata.forkPoint == fork_point
           
           # 验证区块有效性(共识证明、交易、状态转换)
           assert validate_block(block, selected_chain['consensus']) == true
           
           # 验证哈希链连续性
           if h > fork_point:
               prev_block = selected_fork.get_block(h - 1)
       ...
       atomic {
           ...
           # 2.2 将主链的 head 指针切换到选中的候选链
           #     ⚠️ 候选链的区块内容完全不变(包括 upgradeMetadata)
           main_chain.head = selected_fork.head
           main_chain.tip_hash = selected_fork.tip_hash
           main_chain.consensus_id = selected_fork.consensus_id
           
           # 2.3 更新区块高度索引
           #     将 [fork_point, switch_height] 高度映射到选中的候选链区块
           for h in range(fork_point, switch_height + 1):
               selected_block = selected_fork.get_block(h)
               # 注意: selected_block 保持原样,不做任何修改
               # selected_block.header.upgradeMetadata.isPreexec 仍然是 true
               main_chain.height_to_block[h] = selected_block
           
           # 2.4 清理其他未选中的候选链
           for candidate_id, chain_data in other_chains.items():
               if candidate_id != selected_id:
                   delete_candidate_chain(candidate_id, chain_data['chain'])
                   log.info(f"已清理未选中的候选链: {candidate_id}")
           
           # 2.5 切换共识引擎
           consensus_engine.stop(old_consensus)
           consensus_engine.start(selected_chain['consensus'])
       }
       
       # 3. 验证切换后的链状态
       assert main_chain.get_block(switch_height).header.consensusID == selected_chain['consensus'].id
       assert main_chain.state_root == selected_fork.state_root
       ...
       log.info(f"升级成功: 主链已切换到选中的候选共识 {selected_id},高度 {switch_height}")
       log.info(f"注意: 区块内容未修改,upgradeMetadata 保持原样用于历史审计")
   ```

3. **激励分配**:
   - 向参与签名的委员会成员分配奖励
   - 记录升级历史到链上,包括选中的 candidateID
...
回退流程:
```python
def rollback_upgrade(proposal_id: Hash, candidate_chains, reason: String):
    # 停止所有候选链
    for candidate_id, chain_data in candidate_chains.items():
        stop_candidate_chain(candidate_id, chain_data)
    
    # 删除所有候选链数据
    for candidate_id, chain_data in candidate_chains.items():
        delete_candidate_chain(proposal_id, candidate_id, chain_data['chain'])
...
3. **多链安全隔离 (Multi-Chain Isolation)**:
   - 预执行链的失败不影响主链安全性
...
| 预执行阶段 | O(T_preexec) | 多链并行,时间约为 T_preexec |
| 确认验证 | O(t) | 验证 t 个批准签名 |
| 切换操作 | O(1) | 原子操作 |
...
本文提出的共识可切换升级协议 (SCUP) 通过多链预执行、治理多签和 CDL 描述语言,实现了区块链共识的安全在线升级。协议具有以下优势:

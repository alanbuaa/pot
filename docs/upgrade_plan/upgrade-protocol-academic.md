...
本文提出一种支持在线热升级的区块链共识协议切换机制(Switchable Consensus Upgrade Protocol, SCUP)。该协议允许区块链系统在运行时安全地从一种共识算法切换到另一种共识算法,无需停机或硬分叉。通过引入升级配置交易、双链预执行机制、治理委员会多签验证和共识描述语言(Consensus Description Language, CDL),本协议实现了共识算法的可插拔升级,同时保证系统的安全性、活性和一致性。
...
3. **治理委员会多签验证**: 基于门限签名的去中心化治理机制  // 这里待定
...
- **数字签名不可伪造性**: 基于EUF-CMA (Existential Unforgeability under Chosen Message Attack)
...
- **超时参数**: 
  - T_prepare: 预备阶段的最小区块高度 (≥ 100)
  - T_preexec: 预执行阶段的最小区块数 (≥ 1000)
...
UpgradeConfirmTx := {
    ...
    preexecMetrics: ExecutionMetrics,    // 预执行性能指标
...
**切换后的主链区块** :
注意：区块本身完全不变,只是节点的"主链头指针"指向了这条链
...
1. **直接切换**: 预执行链区块保持原样,通过链指针切换实现激活
2. **哈希不变**: 区块内容不修改,哈希链保持完整,共识证明保持有效
3. **状态一致**: 相同的 `stateRoot` 确保状态转换一致
4. **简化验证**: 验证逻辑统一,无需区分主链和预执行链
5. **历史可追溯**: `upgradeMetadata` 字段完整保留,清晰记录升级历史
6. **共识中立**: `consensusProof` 设计支持任意共识类型
7. **原子切换**: 通过指针操作实现瞬间切换,无中间状态
...
| 字段 | 激活前的含义 | 激活后的含义 |
|------|------------|------------|
| `isPreexec=true` | "此区块在预执行链中" | "此区块来自预执行阶段(历史标记)" |
| `proposalID` | 关联的升级提案 | 追溯此升级的提案ID |
| `forkPoint` | 预执行链分叉点 | 追溯升级发生的高度 |
| `mainChainRef` | 对应的主链区块 | 当时并行运行的主链区块(历史记录) |

通过改变**字段的语义解释**而非修改字段内容,实现无损升级。
...
2. **组件兼容性**: 所有引用的密码学组件在系统中可用
3. **状态机正确性**: 状态转换无死锁,所有状态可达
4. **安全属性可满足性**: 声明的安全属性在理论上可验证
5. **资源约束**: 不超过系统资源限制(内存、CPU、网络)
6. **容错模型兼容性**: 验证新旧共识的容错约束是否兼容
...
1. **性能评估**:
   ```python
   def evaluate_preexecution(proposal_id: Hash) -> bool:
       metrics = get_all_metrics(proposal_id)
       
       # 计算聚合指标
       avg_block_time = average(metrics.block_times)
       avg_throughput = average(metrics.throughputs)
       error_rate = sum(metrics.errors) / len(metrics)
       
       # 检查是否满足要求
       if error_rate > 0.05:  # 错误率超过 5%
           return False
       if avg_block_time > target_block_time * 1.5:  # 超时 50%
           return False

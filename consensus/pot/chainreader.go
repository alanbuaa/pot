...
func (c *ChainReader) GetSelfLockUTXO() map[string][]*LockProof {

	height := c.GetCurrentHeight()

	//totallock := int64(0)

	lockmap := make(map[string][]*LockProof)

	c.sync.RLock()
	defer c.sync.RUnlock()

	for unlockheight, lockedUTXOs := range c.unlockUtxo {
		if unlockheight >= height {
			for s, output := range lockedUTXOs {
				if !output.IsCoinbase && len(output.Address) != 0 {
					var txheight uint64
					var txid string
					n, err := fmt.Sscanf(s, "%s:%d", &txid, &txheight)
					if err != nil || n != 2 {
						return nil
					}
					remainlocktime := txheight + output.LockTime - height

					proof := types.DecodeByte2Proof(output.Proof)
					lockproof := &LockProof{
					...
					if _, exist := lockmap[addressString]; !exist {
						lockmap[addressString] = make([]*LockProof, 0)
					}
					proofs := lockmap[addressString]
					proofs = append(proofs, lockproof)
					lockmap[addressString] = proofs
				}
			}
		}
	}

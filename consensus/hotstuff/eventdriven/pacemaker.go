...
func (p *pacemakerImpl) UpdateHighQC(qcHigh *pb.QuorumCert) {
	...
	if block.Height > oldQCHighBlock.Height {
		p.log.WithFields(logrus.Fields{
			"old_height": oldQCHighBlock.Height,
			"new_height": block.Height,
		}).Debug("Updated high QC and leaf block")
		p.ehs.qcHigh = qcHigh
		p.ehs.bLeaf = block
	}
}
...
func (p *pacemakerImpl) OnNextSyncView() {
	p.log.WithField("old_view", p.ehs.View.ViewNum).Warn("New view timeout triggered")
	// view change
	p.ehs.View.ViewNum++
	p.ehs.View.Primary = p.ehs.GetLeader()
	p.log.WithFields(logrus.Fields{
		"new_view":   p.ehs.View.ViewNum,
		"new_leader": p.ehs.View.Primary,
	}).Debug("View changed")
	...
	p.log.Debug("Created and stored dummy block for view change")

	// create a new view msg
	newViewMsg := p.ehs.Msg(pb.MsgType_NEWVIEW, nil, p.ehs.GetHighQC())
	// send msg
	if p.ehs.ID != p.ehs.GetLeader() {
		p.log.WithField("leader", p.ehs.GetLeader()).Debug("Sending NEWVIEW message to leader")
...
func (p *pacemakerImpl) OnReceiverNewView(qc *pb.QuorumCert) {
	...
	p.log.Debug("Received NEWVIEW message")
...
func (p *pacemakerImpl) startNewViewTimeout(closed chan []byte) {
	for {
		...
		case <-p.ehs.TimeChan.Timeout():
			...
			p.log.WithFields(logrus.Fields{
				"old_timeout": p.ehs.Config.HotStuff.Timeout,
				"new_timeout": p.ehs.Config.HotStuff.Timeout * 2,
			}).Debug("Timeout occurred, doubling timeout duration")
		...
		case <-p.ehs.BatchTimeChan.Timeout():
			p.log.Debug("Batch timeout triggered")

...
1. 参考cmd/server和cmd/client目录下的代码，帮我实现一个针对可升级共识upgrade的节点程序和客户端程序，这两个程序的配置文件都已经放到对应的目录下。
2. 将consensus/upgrade下的可自定义多链共识整合到consensus/upgradableConsensus.go中，使得客户端提交提案升级之后能够让节点程序切换共识。
3. 客户端除了能不断发起测试交易之外，还能够发起共识切换的提案和确认投票，为了测试，只有一个确认投票即可。
4. test.sh
   最后通过test.sh脚本来完成节点、客户端启动，客户端不断发交易、客户端发起切换提案、客户端发起确认提案、节点完成在预定的高度切换共识的整个流程测试。
5. 测试过程中，打印出关键的信息，包括双链执行结构信息、共识监控数据等。
